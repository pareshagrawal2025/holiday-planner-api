...
# This script will build docker image for holiday-planner-api service with tag 2.0.0
# The image name and version can be changed as per the requirement
# Example image name after successful build: holidayplanner.example.com/api/holiday-planner-api:2.0.0
...
if [ -f ${DOCKER} ]; then
  ${DOCKER} build -t holidayplanner.example.com/api/holiday-planner-api:2.0.0 .
else
